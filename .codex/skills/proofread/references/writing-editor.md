# Writing Editor — subagent brief

> **Canonical definition lives in `.codex/agents/writing-editor.toml`** (registered as `[agents.writing_editor]` in `.codex/config.toml`). This file is a readable copy for browsing; Codex loads the role from the TOML, and the `proofread` skill spawns it by `agent_role: "writing_editor"`. If you edit the persona, edit the TOML (and mirror here).
